    ...
    private void runMethod(String url, ResponseCollector responseCollector) {
        ...
        if (isCrumbDataSet()) {
            if (version.equals(Version.VER_1)) {
                ...
                com.intellij.openapi.diagnostic.Logger.getInstance(DefaultSecurityClient.class).info("LACZE PRZEZ VERSJE 1");
                com.intellij.openapi.diagnostic.Logger.getInstance(DefaultSecurityClient.class).debug("LACZE PRZEZ VERSJE 1");
            } else {
                post.addRequestHeader(CRUMB_NAME_VER2, crumbData);
                com.intellij.openapi.diagnostic.Logger.getInstance(DefaultSecurityClient.class).info("LACZE PRZEZ VERSJE 2");
                com.intellij.openapi.diagnostic.Logger.getInstance(DefaultSecurityClient.class).debug("LACZE PRZEZ VERSJE 2");
            }
        }
